...
#else
#include "NameToUnicodeTableConst.h"
#endif
...
GlobalParams::GlobalParams(char *cfgFileName) {
  ...
  baseDir = new GString();
#else
  baseDir = appendToPath(getHomeDir(), (char *)".xpdf");
#endif
...
#else
  for (unsigned int x = 0; x < NAME_TO_UNICODE_TAB_LENGTH; ++x) {
    nameToUnicode->add(nameToUnicodeTabString + nameToUnicodeTabConst[x].offs, nameToUnicodeTabConst[x].u); 
  }
#endif
  ...
  if (!f) {
    fileName = appendToPath(getHomeDir(), (char *)xpdfUserConfigFile);
    if (!(f = fopen(fileName->getCString(), "r"))) {
      delete fileName;
    }
  }
...
GlobalParams::~GlobalParams() {
  ...
  while (cMapDirs->getNext(&iter, &key, (void **) &vdPtr/*&list*/)) {
...
FcPattern *buildFcPattern(GfxFont *font)
{
  ...
  if (font->isCIDFont())
  {
    GString *collection = ((GfxCIDFont *)font)->getCollection();
    if (collection)
    {
...
CharCodeToUnicode *GlobalParams::getUnicodeToUnicode(GString *fontName) {
  CharCodeToUnicode *ctu;
  GHashIter *iter;
  GString *fontPattern, *fileName=NULL;
  void *vdPtr = (void *)fileName;

  lockGlobalParams;
  fileName = NULL;
  unicodeToUnicodes->startIter(&iter);
  while (unicodeToUnicodes->getNext(&iter, &fontPattern, (void **) &vdPtr/*&fileName*/)) {
