...
void GfxFont::readFontDescriptor(XRef *xref, Dict *fontDict) {
  ...
  if (fontDict->lookup("FontDescriptor", &obj1)->isDict()) {
    ...
    if (obj2.isString()) family = new GString(obj2.getString());
    obj2.free();
  ...
  GfxFont(tagA, idA, nameA)
{
  ...
  if (obj1.isDict()) {
    ...
    if (obj2.isName("MacRomanEncoding")) {
      ...
      baseEnc = const_cast<char **>(macRomanEncoding);
    } else if (obj2.isName("MacExpertEncoding")) {
      hasEncoding = gTrue;
      baseEnc = const_cast<char **>(macExpertEncoding);
    } else if (obj2.isName("WinAnsiEncoding")) {
      hasEncoding = gTrue;
      baseEnc = const_cast<char **>(winAnsiEncoding);
    }
    obj2.free();
  ...
  if (!baseEnc) {
    if (builtinFont && embFontID.num < 0) {
      baseEnc = const_cast<char **>(builtinFont->defaultBaseEnc);
      hasEncoding = gTrue;
    } else if (type == fontTrueType) {
      baseEnc = const_cast<char **>(winAnsiEncoding);
    } else {
      baseEnc = const_cast<char **>(standardEncoding);
    }
  }
