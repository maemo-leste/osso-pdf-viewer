...
//VG: change because of fontconfig
...
//VG
  ...
  //----
  GBool getShowImages(void) { return showImages; }
  void  setShowImages(GBool show) { showImages = show; }
  GBool getBigImage(void) { return bigImage; }
  void setBigImage(void) { bigImage=gTrue; }
  void ackBigImage(void) { bigImage=gFalse; }
...
private:
  ...
  FcConfig *FCcfg;		// fontconfig

  GBool showImages;             // show images
  GBool bigImage;               // a huge image was tried to be displayed
...
#if MULTITHREADED
  G_Mutex mutex;
  G_Mutex unicodeMapCacheMutex;
  G_Mutex cMapCacheMutex;
#endif
};
