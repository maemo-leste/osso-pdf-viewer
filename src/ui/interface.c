...
static void ui_set_current_page(AppUIData * app_ui_data,
				gint current_page_number, gint total_pages);
...
void ui_create_main_window(AppData * app_data)
{
	...
	gtk_widget_set_size_request(GTK_WIDGET(popup_widget), -1,
				    POP_MENU_ITEM_WIDTH);
	...
	display =
	    gdk_window_get_display(GTK_WIDGET(app_ui_data->app_view)->window);
	...
	XChangeProperty(GDK_DISPLAY_XDISPLAY(display),
			GDK_WINDOW_XID(GTK_WIDGET(app_ui_data->app_view)->
				       window), atom, XA_INTEGER, 32,
...
void ui_create_menu(AppUIData * app_ui_data)
{
	...
	gtk_button_set_label(GTK_BUTTON(button),
			     _("pdfv_me_menu_document_open"));
	g_signal_connect_after(button, "clicked", G_CALLBACK(on_document_open),
			       app_ui_data);
	hildon_app_menu_append(app_ui_data->menu, GTK_BUTTON(button));
	app_ui_data->menu_open = button;	/* store reference */
	gtk_widget_add_accelerator(button, "activate", accel, GDK_o,
				   GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);

	/* Save */
	button = hildon_gtk_button_new(buttonsize);
	gtk_button_set_label(GTK_BUTTON(button),
			     _("pdfv_me_menu_document_save"));
	g_signal_connect_after(button, "clicked", G_CALLBACK(on_document_save),
			       app_ui_data);
	hildon_app_menu_append(app_ui_data->menu, GTK_BUTTON(button));
	app_ui_data->menu_save = button;	/* store reference */
	gtk_widget_add_accelerator(button, "activate", accel, GDK_s,
				   GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);

	/* Details */
	button = hildon_gtk_button_new(buttonsize);
	gtk_button_set_label(GTK_BUTTON(button),
			     _("pdfv_me_menu_document_details"));
	g_signal_connect_after(button, "clicked",
			       G_CALLBACK(on_document_details), app_ui_data);
	hildon_app_menu_append(app_ui_data->menu, GTK_BUTTON(button));
 ...
 */
void ui_run_file_chooser(AppUIData * app_ui_data, GtkFileChooserAction action)
{
	/* construct uri */
	...
	if ((strncmp(uri_str, "file:///var/tmp/", 16) == 0)	/* for files opened from web */
		...
		char *path =
		    g_build_filename(get_factory_default_folder(), filename,
				     NULL);
	...
	if (filter != NULL) {
		gtk_file_chooser_add_filter(GTK_FILE_CHOOSER(cos->fc_dialog),
					    filter);
		gtk_file_chooser_set_filter(GTK_FILE_CHOOSER(cos->fc_dialog),
					    filter);
	}
		...
		if (filename != NULL) {
			if (action == GTK_FILE_CHOOSER_ACTION_OPEN) {
				...
				if (result == RESULT_SAVE_OK) {
					ui_open_document(app_ui_data, filename,
							 NULL);
					//ui_update(app_ui_data);
				}
	...
	do {
		if (retries == 0) {
			...
			if (response != GTK_RESPONSE_OK) {
				...
				if (gpd)
					ui_enable_document_open(app_ui_data,
								TRUE);
		...
		if (retries == 0) {
			...
			/* bug: 87157 STARTS */
			g_idle_add((GSourceFunc) encrypt_file_banner,
		 ...
		 const gchar * filename, const gchar * password)
{
	...
	if (app_ui_data->app_data->state != PDF_VIEWER_STATE_SAVING) {
		...
		idle_id =
		    g_idle_add_full(G_PRIORITY_DEFAULT_IDLE, idle_open, p,
				    idle_delete);
	}
}
...
void ui_run_switch_page_dialog(AppUIData * app_ui_data)
{
	 ...
	 * page */
	text = g_strdup_printf(PDFstr	/* SUPPRESS_FORMAT_WARNING
					   (_("pdfv_fi_switch_page_amount")) */ , value,
			       max);
	...
	caption_control = hildon_caption_new(NULL,
					     _
					     ("pdfv_fi_switch_page_description"),
					     number_editor, NULL,
	...
	do {
		...
		idle_id =
		    (gint) g_object_get_data(G_OBJECT(number_editor),
					     "idle-id");
...
void ui_run_details_dialog(AppUIData * app_ui_data)
{
	...
	ComappDetailsData cdd = {
		...
		.fs_model =
		    get_file_system_model(GTK_WIDGET(app_ui_data->app_view)),
...
void ui_calculate_arrows_cords(AppUIData * app_ui_data, Arrow_Cords * cords)
{
	...
	cords->x_pos_right =
	    scrollbar_pos + window_width - NAVIGATE_IMAGE_WIDTH;
}
...
void ui_calculate_overlay_cords(AppUIData * app_ui_data, Arrow_Cords * cords)
{
	...
	window_height = GTK_WIDGET(app_ui_data->layout)->allocation.height;
	window_width = GTK_WIDGET(app_ui_data->layout)->allocation.width;
	cords->y_pos = scrollbar_pos + (window_height - NAVIGATE_IMAGE_HEIGHT);
	...
	cords->x_pos_right =
	    scrollbar_pos + window_width / 2 + NAVIGATE_IMAGE_WIDTH / 2;
}
	     ...
	     gint dest_y, GdkPixbuf ** temporary_copy)
{
	...
	arrowdraw = gdk_pixbuf_get_from_drawable(NULL,
						 GTK_WIDGET(app_ui_data->
							    page_image)->window,
						 NULL, dest_x, dest_y, 0, 0,
						 NAVIGATE_IMAGE_WIDTH,
						 NAVIGATE_IMAGE_HEIGHT);
...
gboolean ui_put_left_arrow_on_idle(gpointer data)
{
	...
	ui_put_arrow(app_ui_data, app_ui_data->left_image,
		     arrow_cords.x_pos_left, arrow_cords.y_pos,
		     &app_ui_data->left_image_orig);
...
static gboolean ui_put_arrows_idle(gpointer data)
{
	...
	if (app_ui_data->zooming_banner == NULL) {
		...
		if (!(pdf_viewer_is_rendering())) {
			ui_put_arrow(app_ui_data, app_ui_data->ovr_image,
				     arrow_cords.x_pos_left, arrow_cords.y_pos,
				     &app_ui_data->ovr_image_orig);
			app_ui_data->ovrshown = TRUE;
		}
	}
#endif
...
void ui_update(AppUIData * app_ui_data)
{
	...
	if (cur_state == PDF_VIEWER_STATE_EMPTY) {
		...
		if (g_str_has_suffix(file, PDFV_FILE_EXTENSION_DOT)) {
			title[strlen(title) - strlen(PDFV_FILE_EXTENSION_DOT)] =
			    '\0';
		}
		g_free(file);
...
add_custom_actions(GtkActionGroup * actions, AppUIData * app_ui_data)
{
	...
	g_signal_connect(G_OBJECT(action),
			 "activate", G_CALLBACK(on_page_switch_to),
			 app_ui_data);
				 ...
				 AppUIData * app_ui_data)
{
	...
	for (i = 0; i < nitems; i++) {
		...
		if (!GTK_IS_SEPARATOR_TOOL_ITEM(tool_item)) {
			...
			gtk_widget_set_can_focus(gtk_bin_get_child
						 (GTK_BIN(tool_item)), FALSE);
			...
			if (GTK_IS_TOOL_BUTTON(tool_item)
				...
				if (image != NULL) {
					...
					gtk_tool_button_set_icon_widget
					    (GTK_TOOL_BUTTON(tool_item), image);
				}
	...
	gtk_button_set_alignment(GTK_BUTTON(button), (gfloat) 1.0,
				 (gfloat) 0.5);
	...
	app_ui_data->img_images_off =
	    gtk_image_new_from_icon_name
	    ("pdf_viewer_no_images", HILDON_ICON_SIZE_TOOLBAR);

	g_object_ref(app_ui_data->img_images_off);

	app_ui_data->fullscreen_toolbar_button =
	    GTK_WIDGET(gtk_toolbar_get_nth_item
		       (toolbar, TOOLBAR_POS_FULLSCREEN));
...
static void build_application_area(AppUIData * app_ui_data)
{
	...
	g_assert(app_ui_data->right_image && app_ui_data->left_image
		 && app_ui_data->ovr_image);
...
*/
static void
ui_set_current_page(AppUIData * app_ui_data, gint current_page_number,
		    gint total_pages)
{
	gchar *text =
	    g_strdup_printf("%d/%d", current_page_number, total_pages);
...
void ui_show_note(AppUIData * app_ui_data, const gchar * msg)
{
	...
	if (msg) {
		app_ui_data->note_dialog = note =
		    hildon_note_new_information(GTK_WINDOW
						(app_ui_data->app_view), msg);
...
void ui_show_result(AppUIData * app_ui_data, PDFViewerResult result)
{
	...
	case RESULT_UNSUPPORTED_FORMAT:
		ui_show_note(app_ui_data, _("pdfv_ni_unsupported_file"));
		break;
	case RESULT_CORRUPTED_FILE:
		ui_show_note(app_ui_data, _("pdfv_error_documentcorrupted"));
		break;
	case RESULT_INSUFFICIENT_MEMORY:
		ui_show_note(app_ui_data, _("pdfv_ni_not_enough_memory"));
		break;
