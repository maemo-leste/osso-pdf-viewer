...
static gboolean key_press_disabled(AppUIData * app_ui_data, GdkEventKey * event)
{
	/* 
...
gboolean key_press(GtkWidget * widget, GdkEventKey * event, gpointer data)
{
	...
	case GDK_Up:
		...
		adj =
		    gtk_layout_get_vadjustment(GTK_LAYOUT(app_ui_data->layout));
		...
		if (adj->upper <= adj->page_size || adj->value == 0.0) {
			...
			if ((w != NULL) && (gtk_widget_is_sensitive(w))) {
				...
				if (pdf_viewer_get_current_page() == 1) {
					ui_show_banner(GTK_WIDGET
						       (app_ui_data->app_view),
						       _
						       ("pdfv_ib_first_page_reached"));
				}
			}
...
gboolean key_release(GtkWidget * widget, GdkEventKey * event, gpointer data)
{
	...
	case GDK_Return:
		...
		if ((key_pressed_was == GDK_KP_Enter || key_pressed_was == GDK_Return) && (pdf_viewer_get_num_pages() == 0)) {	/* &&
...
/* If scroll is far left/top or page fits to screen then try to move to previous page */
		...
		if (adj->upper <= adj->page_size || adj->value < 0.0001) {
			w = gtk_ui_manager_get_widget(app_data->app_ui_data->
						      ui_manager,
						      "/ToolBar/pdfv_me_menu_page_previous");
			if ((w != NULL) && (gtk_widget_is_sensitive(w))) {
				pdf_viewer_navigate_page
				    (pdf_viewer_get_current_page() - 1);
...
gboolean
on_scroll_press(GtkWidget * widget, GdkEventButton * event, gpointer user_data)
{
	AppUIData *app_ui_data = (AppUIData *) user_data;
	ui_hide_overlay_image(app_ui_data, app_ui_data->ovr_image_orig);
...
gboolean
on_screen_press(GtkWidget * widget, GdkEventButton * event, gpointer user_data)
{
	AppUIData *app_ui_data = (AppUIData *) user_data;
	g_return_val_if_fail(app_ui_data != NULL, TRUE);
		  ...
		  gpointer user_data)
{
	...
	if (PDF_FLAGS_IS_SET(app_ui_data->flags, PDF_FLAGS_FULLSCREEN)) {
		...
		if (app_ui_data->app_data->state == PDF_VIEWER_STATE_EMPTY) {
			app_ui_data->press_lastx = app_ui_data->lastx =
			    (gint) event->x;
			app_ui_data->press_lasty = app_ui_data->lasty =
			    (gint) event->y;
		}
		...
		} else if ((abs(app_ui_data->press_lastx - event->x) <
			    MOVE_THRESHOLD)
			   && (abs(app_ui_data->press_lasty - event->y) <
			       MOVE_THRESHOLD)) {

			//if (press_pos >= (scrollbar_pos + SCREEN_WIDTH - ACTIVE_AREA_WIDTH)) {
			if (!app_ui_data->isPortrait) {
				if (press_pos >=
				    (scrollbar_pos + SCREEN_WIDTH -
				     ACTIVE_AREA_WIDTH)
				    && (press_pos_fs <=
					(v_scrollbar_pos + SCREEN_HEIGHT -
					 ACTIVE_AREA_HEIGHT))) {
					move = 1;
				} else if (press_pos <=
					   (scrollbar_pos +
				...
				if (press_pos >=
				    (scrollbar_pos + SCREEN_HEIGHT -
				     ACTIVE_AREA_WIDTH)
				    && (press_pos_fs <=
					(v_scrollbar_pos + SCREEN_WIDTH -
					 ACTIVE_AREA_HEIGHT))) {
					move = 1;
				} else if (press_pos <=
					   (scrollbar_pos +
...
gboolean
on_screen_motion(GtkWidget * widget, GdkEventMotion * event, gpointer user_data)
{
	AppUIData *app_ui_data = (AppUIData *) user_data;
	GtkAdjustment *adj;
	gdouble value;
	...
	if (!PDF_FLAGS_IS_SET(app_ui_data->flags, PDF_FLAGS_PANNING)) {
		...
		adj =
		    gtk_layout_get_hadjustment(GTK_LAYOUT(app_ui_data->layout));
...
gboolean
window_state_changed(GtkWidget * widget, GdkEventWindowState * event,
		     gpointer user_data)
{
	AppUIData *app_ui_data = (AppUIData *) user_data;
...
gboolean show_number_editor_error(gpointer data)
{
	...
	guint min = PDF_PAGE_INIT + 1, max = pdf_viewer_get_num_pages();
	const gchar *PDFstr =
	    SUPPRESS_FORMAT_WARNING(_("pdfv_error_pagenumber"));
	...
	hildon_banner_show_information(GTK_WIDGET
				       (number->app_ui_data->app_view), NULL,
				       info);
	g_free(info);
		...
		GnomeVFSMonitorEventType event_type, gpointer user_data)
{
	...
	case GNOME_VFS_MONITOR_EVENT_DELETED:
		...
		if (app_data->state == PDF_VIEWER_STATE_LOADED) {
			...
			if (strcmp(monitor_uri, loaded_uri) == 0) {
				/* Dim 'save as' item */
				widget =
				    gtk_ui_manager_get_widget(app_data->
							      app_ui_data->ui_manager,
							      "/MenuBar/pdfv_me_main_menu_document/"
	...
	default:
		OSSO_LOG_DEBUG
		    ("file has been something! info: %s, monitor: %s\n",
		     info_uri, monitor_uri);
		break;
	}
}
